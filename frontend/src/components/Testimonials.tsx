...
const Testimonials = () => {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        setTestimonials([]); // Reset to empty array on error
      } finally {
        setLoading(false);
      }
    };
  ...
  // Fallback data in case API fails or is not set up yet
  ...
  const displayTestimonials = loading && testimonials.length === 0 ? fallbackTestimonials : testimonials;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-12 space-y-8">
            <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4">
              ...
              <p className="mt-4">
                Showing sample testimonials instead. Please check your API connection.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="border-t pt-12">
          ...
          <div className="grid gap-8 sm:grid-cols-3 lg:grid-cols-6">
            {/* Using fallback logos since API doesn't have logo endpoint yet */}
            {[1, 2, 3, 4, 5, 6].map((logo, index) => (
              <div key={index} className="flex items-center justify-center">
                <img
                  src={`https://via.placeholder.com/150x80/${['2563eb', 'dc2626', '059669', '7c3aed', 'ea580c', '16a34a'][index]}/ffffff?text=LOGO${index + 1}`}
